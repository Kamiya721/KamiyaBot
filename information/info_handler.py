...
async def group_msg_handle(message):
  try:
    ...
    elif module_name == 'codeforces':
      from codeforces.UserSearch import PrintImage
      match_res = re.match(r'^\s*#(\w+)\s+(\S+)', raw_message)
      if not match_res:
        return
      username = match_res.group(2)
      data = await PrintImage(username)
      if data:
        await send_group_message(group_id, data)
      else:
        await send_group_message(group_id, "User not found.")
  except Exception as e:
